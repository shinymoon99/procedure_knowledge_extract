...
class C2F(torch.nn.Module):
	...
	def train(self, train_x1, train_y1, test_x1,  test_y1, train_x2, train_y2, test_x2,  test_y2, train_x3s, train_x3w, train_y3, test_x3s,  test_x3w,  test_y3):
		...
		for epoch in range(2000):
			...
			pre_labels_y1 = [Max_Index(line) for line in prediction_test.data.numpy()]
			recall, precision, macrof1, microf1, acc = Get_Report(test_y1, pre_labels_y1)
			print( "[{:4d}]    recall:{:.4%}    precision:{:.4%}    macrof1:{:.4%}    microf1:{:.4%}    accuracy:{:.4%}    loss:{:.4%}".format(epoch, recall, precision, macrof1, microf1, acc,loss))
		...
		for epoch in range(2000):
			...
			pre_labels_y2 = [Max_Index(line) for line in prediction_test.data.numpy()]
			recall, precision, macrof1, microf1, acc = Get_Report(test_y2, pre_labels_y2)
			print( "[{:4d}]    recall:{:.4%}    precision:{:.4%}    macrof1:{:.4%}    microf1:{:.4%}    accuracy:{:.4%}    loss:{:.4%}".format(epoch, recall, precision, macrof1, microf1, acc,loss))
		...
		for epoch in range(2000):
			...
			pre_labels_y3 = [Max_Index(line) for line in prediction_test.data.numpy()]
			recall, precision, macrof1, microf1, acc = Get_Report(test_y3, pre_labels_y3)
			print(
				"[{:4d}]    recall:{:.4%}    precision:{:.4%}    macrof1:{:.4%}    microf1:{:.4%}    accuracy:{:.4%}    loss:{:.4%}".format(
					epoch, recall, precision, macrof1, microf1, acc, loss))

		return pre_labels_y1,pre_labels_y2,pre_labels_y3,loss1,loss2,loss3